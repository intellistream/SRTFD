        ...
        if params.online:
            ...
            for i, (x_train, y_train, labels) in enumerate(data_continuum):
                ...
                if params.agent == 'SRTFD':
                    acc_array, rec_array, pre_array, f1_array, gmean_array, latencies, ttl_time = agent.evaluate(
                ...
                to_csv.append([np.percentile(latencies, 95), ttl_data / ttl_time, (memory_after + memory_before) / 2])
                ...
                if params.agent == 'SRTFD':
                    agent.train_learner(
                        x_train, y_train, pseudo_x=agent.pseudo_x, pseudo_y=agent.pseudo_y, alpha=0.7, coreset_ratio=0.6)
                else:
                    agent.train_learner(
                        x_train, y_train)
            ...
            with open(f"{params.data}_{params.agent}_{params.cl_type}_run_{run}_task.csv", mode='w', newline='') as file: 
                writer = csv.writer(file)
                writer.writerow(["Latency (ns)", "Throughput (/s)", "Memory Usage (MB)"])
