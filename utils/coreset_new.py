...
class CoresetGreedy:
    ...
    def sample(self, sample_ratio, buffer):
        ...
        per_class_sample_size = sample_size // num_classes

        print('max per class sample: ', per_class_sample_size,
              'num class: ', num_classes)

        new_batch = {label: [] for label in unique_labels}

        for _ in range(self.dset_size):
            ...
            m = min([len(val) for _, val in new_batch.items()])

            if m > per_class_sample_size:
                break
        ...
        print("sample_size:{}".format(len(new_balanced_batch)))
