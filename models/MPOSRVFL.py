...
class node_generator:
    ...
    def generator_nodes(self, data, times, batchsize, nonlinear):  # 将特征结点和增强结点构建起来
        self.Wlist = [elem[0] for elem in self.generator(
            (data.shape[1], batchsize), times)]
        self.blist = [elem[1] for elem in self.generator(
            (data.shape[1], batchsize), times)]
        ...
        for i in range(1, len(self.Wlist)):
            nodes = np.column_stack((nodes, self.nonlinear(
                data.dot(self.Wlist[i]) + self.blist[i])))
        return nodes

    def transform(self, testdata):
        testnodes = self.nonlinear(testdata.dot(self.Wlist[0]) + self.blist[0])
        for i in range(1, len(self.Wlist)):
            testnodes = np.column_stack((testnodes, self.nonlinear(
                testdata.dot(self.Wlist[i]) + self.blist[i])))
        return testnodes
...
class MPOS_RVFL:
    def __init__(self, Ne, N2, enhence_function, reg, gamma, n_anchor, sigma):
        ...
        self.onehotencoder = preprocessing.OneHotEncoder(sparse_output=False)
        ...
        for i in range(n):
            for j in range(d):
                if j in np.argsort(distance_matrix[i])[:k]:
                    s_matrix[i, j] = np.exp(-np.linalg.norm(X[i] - K[j])
                                            ** 2 / (2 * sigma ** 2))
    ...
    def fit(self, X, Y):
        ...
        Y_enc = self.onehotencoder.fit_transform(Y.reshape(-1, 1))
        H = self.enhence_generator.generator_nodes(
            X_enc, self._Ne, self._N2, self._enhence_function)
        ...
        if count_min >= int(self.anchor/len(Y_types)):
            select_index = []
            for p in Y_types:
                select_index.extend(np.random.choice(
                    np.where(Y == p)[0], int(self.anchor/len(Y_types)), replace=False))
        # If data imbalance is severe, priority will be given to minority class
            ...
            for p in Y_types:
                if p != 0:
                    select_index.extend(np.random.choice(
                        np.where(Y == p)[0], count_min, replace=False))
            remain_num = self.anchor - len(select_index)
            select_index.extend(np.random.choice(
                np.where(Y == 0)[0], remain_num, replace=False))
    ...
    def partial_fit(self, X_at, Y_at=None):
        ...
        self.cal_weight()
        T_at = self.onehotencoder.transform(pseudo_label.reshape(-1, 1))
